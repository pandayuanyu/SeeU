...
def draw_tracks_2d_static(
    ...
    canvas = (img.cpu().numpy() * 255.0).astype(np.uint8)

    tracks_2d_np = tracks_2d.cpu().numpy()  # (P, T, 2)
    num_tracks, num_frames = tracks_2d_np.shape[:2]

    cmap = colormaps.get_cmap(cmap_name)

    for j in range(num_tracks):
        rgb_float = np.array(cmap(
            j / max(1.0, float(num_tracks - 1))
        )[:3])  # (r,g,b) in [0,1]
        color_bgr_255 = tuple((rgb_float * 255.0).astype(np.int32).tolist())[::-1]


        for t in range(num_frames - 1):
            p1 = tracks_2d_np[j, t]
            p2 = tracks_2d_np[j, t + 1]
