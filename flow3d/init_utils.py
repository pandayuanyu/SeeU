...
def init_motion_params_with_procrustes(
    ...
    dists_th = torch.quantile(dists, 0.98)
    ...
    ids = ids[counts > 10]
    num_bases = len(ids)
    sampled_centers = sampled_centers[:, ids.to(sampled_centers.device)]
    ...
    for n, cluster_id in enumerate(ids):
        ...
        cluster = tracks_3d.xyz[mask_in_cluster.to(tracks_3d.xyz.device)].transpose(
            0, 1
        )  # [num_frames, n_pts, 3]
        visibilities = tracks_3d.visibles[mask_in_cluster.to(tracks_3d.xyz.device)].swapaxes(
            0, 1
        )  # [num_frames, n_pts]
        confidences = tracks_3d.confidences[mask_in_cluster.to(tracks_3d.xyz.device)].swapaxes(
            0, 1
        )  # [num_frames, n_pts]
...
def sample_initial_bases_centers(
    ...
    sampled_centers = torch.stack(
        [
            means_canonical[torch.tensor(labels == i, device=means_canonical.device)].median(dim=0).values
