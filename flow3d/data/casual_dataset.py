...
class CasualDataset(BaseDataset):
    def __init__(
        ...
        **_,
        ...
        self.img_dir = f"{data_dir}/{image_type}/{scene}"
        self.img_ext = os.path.splitext(os.listdir(self.img_dir)[0])[1]
        self.depth_dir = f"{data_dir}/{depth_type}/{scene}"
        self.mask_dir = f"{data_dir}/{mask_type}/{scene}"
        self.tracks_dir = f"{data_dir}/{track_2d_type}/{scene}"
        self.cache_dir = f"{data_dir}/flow3d_preprocessed/{scene}"
        ...
        if camera_type == "droid_recon":
            ...
            w2cs, Ks, tstamps = load_cameras(
                f"{data_dir}/{camera_type}/{scene}.npy", H, W
            )


        # Replace the old `elif camera_type == "megasam"` block
        elif camera_type == "megasam":
            # Get the actual size of the high-resolution images we are loading
            img = self.get_image(0)
            H, W = img.shape[:2]  # e.g., H=1080, W=1920

            # Load the .npz file
            cam_path = Path(data_dir) / f"{scene}_sgd_cvd_hr.npz"
            cams = np.load(cam_path)
    
            c2ws = cams["cam_c2w"][:self.end]
            K_lowres = cams["intrinsic"]  # This is the low-resolution intrinsic matrix
            depths_lowres = cams["depths"]  # Load the depth maps from the npz

            # --- add logging ---
            guru.info("--- [MegaSAM] Before camera parameter scaling ---")
            guru.info(f"High-resolution image size (H, W): ({H}, {W})")
            guru.info(f"Low-resolution intrinsics (K_lowres):\n{K_lowres}")
            # Compute w2c from c2w and print a sample
            w2c_before_sample = torch.linalg.inv(torch.from_numpy(c2ws[0])).numpy()
            guru.info(f"Example extrinsics (w2c) for the first frame:\n{w2c_before_sample}")
            
            # --- CORE MODIFICATION: Derive h, w from the depth map dimensions ---
            # The shape of depths_lowres should be (num_frames, h, w)
            if depths_lowres.ndim == 3 and depths_lowres.shape[0] > 0:
                h, w = depths_lowres.shape[1], depths_lowres.shape[2]
                guru.info(f"Derived MegaSAM low-res dimensions from depth map: h={h}, w={w}")
            else:
                # Fallback to inference from intrinsics if depth data is not as expected
                w, h = int(K_lowres[0, 2] * 2), int(K_lowres[1, 2] * 2)
                guru.warning(
                    "Could not get shape from 'depths' key in npz. "
                    f"Falling back to inference from intrinsics: h={h}, w={w}"
                )
    ...
    # --- Scale intrinsics ---
            ...
            K_highres = K_lowres.copy()
            K_highres[0, 0] *= sx  # Scale fx
            K_highres[0, 2] *= sx  # Scale cx
            K_highres[1, 1] *= sy  # Scale fy
            K_highres[1, 2] *= sy  # Scale cy
    # --- End scaling ---

            c2ws = torch.from_numpy(c2ws).float()
            w2cs = torch.linalg.inv(c2ws)
    # Use the scaled high-resolution intrinsics
            Ks = torch.from_numpy(K_highres).float().unsqueeze(0).repeat((c2ws.shape[0], 1, 1))

            guru.info("--- [MegaSAM] After camera parameter scaling ---")
            guru.info(f"Example high-resolution intrinsics (K_highres) for the first frame:\n{Ks[0].numpy()}")
            guru.info(f"Example extrinsics (w2c) for the first frame (unchanged):\n{w2cs[0].numpy()}")
            # --- End logging ---
        ...
        if scene_norm_dict is None:
            ...
            if os.path.exists(cached_scene_norm_dict_path) and self.load_from_cache:
                ...
                tracks_3d = self.get_tracks_3d(5000, step=self.num_frames // 4)[0]
    ...
    def get_masks(self, index) -> torch.Tensor:
    ...
    def get_depth(self, index) -> torch.Tensor:
        if self.depths[index] is None:
            # New logic: directly check the value of self.depth_type
            if self.depth_type == "megasam_depth":
                # 1. Get the target high-resolution image size
                target_H, target_W = self.get_image(index).shape[:2]

                # 2. Load the .npz file
                depth_path = Path(self.data_dir) / f"{self.scene}_sgd_cvd_hr.npz"
                depths_data = np.load(depth_path)
                
                # 3. Extract the low-resolution depth map
                depth_low_res = torch.tensor(depths_data["depths"][index]).float()
                
          #      guru.info(f"Original MegaSAM depth map resolution: {depth_low_res.shape}")
                
                # 4. Upsample
                depth_high_res = F.interpolate(
                    depth_low_res.unsqueeze(0).unsqueeze(0),
                    size=(target_H, target_W),
                    mode='bilinear',
                    align_corners=False
                ).squeeze()
                
            #    guru.info(f"Upsampled MegaSAM depth map resolution: {depth_high_res.shape}")
                
                self.depths[index] = depth_high_res
            
            # If it is not megasam_depth, use the original .npy loading logic
            elif self.depth_type in [
                "aligned_depth_anything",
                "aligned_depth_anything_v2",
                "depth_anything",
                "depth_anything_v2",
                ...
                raise ValueError(f"Unknown depth loading mode: {self.depth_type}")
    ...
    def get_tracks_3d(
        ...
        masks = torch.stack([self.get_masks(i) for i in target_idcs], dim=0)
    ...
    def get_bkgd_points(
        ...
        **kwargs,
        ...
        for query_idx in tqdm(query_idcs, desc="Loading bkgd points", leave=False):
            ...
            bg_mask = self.get_masks(query_idx) < 0
